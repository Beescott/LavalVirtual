...
public class AmputedMember : MonoBehaviour
{
    ...
    {
        return (lastPointedObjects.Count > 0);
    }

    void Update()
    {
        if (transform != null && transform.parent != null && transform.parent.GetComponentInChildren<RenderModel>() != null)
            transform.parent.GetComponentInChildren<RenderModel>().gameObject.SetActive(false);
        ...
        foreach (RaycastHit r in hits)
        {
            ...
            if (go.CompareTag("GrabableObject") && go.GetComponentInParent<GrabableObject>() != null && !newPointedObjects.Contains(go))
            {
                ...
                if (!lastPointedObjects.Contains(go))
                {
                    ...
                    {
                            go.GetComponentInParent<GrabableObject>().OnPointed();
        ...
        foreach (GameObject go in lastPointedObjects)
        {
            if (!newPointedObjects.Contains(go))
            {
                ...
                else
                {
                    ...
                    go.GetComponentInParent<GrabableObject>().OnPointedExit();
                }
            }
        }

        if (pointedObjects.Count == 0)
            amputedHand.NotPoint();
